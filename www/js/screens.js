...
(function () {
  ...
  BO.TitleScene.prototype = {
    ...
    draw(ctx, W, H) {
      ...
      const sub = 'SHOOT BALLS & BREAK BRICKS';
      ctx.save();
      ...
      this.btnPlay = BO.R.button3D(ctx, px, py, pw * pulse, ph * pulse, {
        color: '#2fbb2f', text: 'PLAY', size: 46,
      ...
      BO.MODES.forEach((m, i) => {
        ...
        ctx.stroke();
        // name
        BO.R.outlineText(ctx, m.name, cx + cw / 2, cy0 + 22, 20, sel ? '#fff' : '#9aa2c8');
        // icon
        ...
        // per-mode level
        BO.R.outlineText(ctx, 'LV ' + BO.store.levelOf(m.id), cx + cw / 2, cy0 + chh - 20, 19,
          sel ? '#ffe063' : '#8b93b0');
  ...
  BO.makeShop = function () {
    ...
    o.draw = function (ctx, W, H) {
      ...
      BO.R.outlineText(ctx, 'BALLS', W / 2, py + 52, 42, '#fff');
      ...
      BO.SKINS.forEach((s, i) => {
        ...
        if (active) {
          BO.R.outlineText(ctx, 'EQUIPPED', cx, cy + cardH * 0.36, 18, '#6fe06f');
        } else if (owned) {
          BO.R.outlineText(ctx, 'TAP TO USE', cx, cy + cardH * 0.36, 18, '#9aa2c8');
        } else {
      ...
      o.closeR = BO.R.button3D(ctx, W / 2 - 90, py + ph - 26, 180, 62,
        { color: '#e8453c', text: 'CLOSE', size: 28 });
    };
  ...
  BO.makeHelp = function () {
    ...
    o.draw = function (ctx, W, H) {
      ...
      const px = (W - pw) / 2, py = (H - ph) / 2;
      BO.ui.panel(ctx, px, py, pw, ph);
      BO.R.outlineText(ctx, 'HOW TO PLAY', W / 2, py + 52, 38, '#fff');
      const lines = [
      ...
      o.closeR = BO.R.button3D(ctx, W / 2 - 90, py + ph - 26, 180, 62,
        { color: '#2fbb2f', text: 'GOT IT', size: 28 });
    };
    o.pointer = function (type, x, y) {
